...
void loop() {
  lcd.clear();
  if (isOn){
    auto joystick_x = analogRead(analogPin2);
    auto joystick_y = analogRead(analogPin1);
    
    // -1 to 1
    auto joy_x = ((float)joystick_x - 512.0f) / 512.0f;
    auto joy_y = ((float)joystick_y - 512.0f) / 512.0f;
